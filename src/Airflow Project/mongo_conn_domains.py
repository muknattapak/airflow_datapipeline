...
def get_data(**kwargs):
    ...
    collect2 = mydb['domains']
    cursor = collect2.find({})
    list_cur = list(cursor)
    temp_data = pd.DataFrame(list_cur)
    df = temp_data.copy().reset_index(drop=True)
    df = df[['_id', 'crt', 'mdt', 'name', 'domainName', 'type', 'status',
             'hospitals', 'location', 'address', 'parentId']]
    df = pd.concat([df.drop('location', axis=1),
                    df['location'].apply(pd.Series)], axis=1)
    df = pd.concat([df.drop('address', axis=1),
                    df['address'].apply(pd.Series)], axis=1)
    ...
    path = os.path.join(os.getcwd(), "dags/temp/domain.csv")
...
def load_data(**kwargs):
    ...
    data_dict[['crtTH', 'mdtTH']] = data_dict[[
        'crtTH', 'mdtTH']].astype('datetime64[ns]')
    ...
    data_dict.to_sql(name="domains_mt", if_exists='replace',
