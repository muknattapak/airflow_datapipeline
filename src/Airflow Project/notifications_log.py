...
def get_data():
    ...
    for x in range(0,30):
        if(latestId):
            x = collect.find(
                {'userId': '60ebd27136ffa000118983ca', 'type': 4,'_id': {'$gt':ObjectId(latestId.decode())},"from": {"$gte": start}})
        else:
            x = collect.find(
                {'userId': '60ebd27136ffa000118983ca', 'type': 4,"from": {"$gt": start}})
    ...
    df['domainId'] = "60ebd27136ffa000118983ca"
...
def load_summary():
    ...
    day_index = pd.DataFrame({'value':[100]})   
    ...
    first_noti = datetime.strptime('8:50am', '%I:%M%p').strftime("%H:%M:%S")
    ...
    day_index["from"] = pd.to_datetime(datetime.strptime('2022-9-29', '%Y-%m-%d'))
    ...
    newdata = pd.concat([day_index],axis=0)
    ...
    newdata['channel'] = "0"
    newdata['app'] = "0"    
    newdata['userid'] = "60ebd27136ffa000118983ca"
    newdata['domainid'] = "608175dae5cd71001119f3bd"
    newdata['title'] = "แจ้งเตือนค่าสุขภาพผิดปกติ"
    newdata['readed'] = "0"
    newdata['status'] = "0"
    ...
    engine = create_engine(
        'postgresql://airflow:airflow@airflow_postgres_1:5432/postgres')
    newdata.to_sql(name="notifications_log", if_exists='replace',
                     con=engine, chunksize=100)
...
with DAG('create_noti_log',
    ...
    t1 = PythonOperator(
        task_id='t1_summary_load_postgres',
        provide_context=True,
